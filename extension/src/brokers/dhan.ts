...
/**
 * Dhan (web.dhan.co / tv.dhan.co / options.dhan.co) order-window adapter —
 * selector generation v1 (June 2026).
 *
 * Dhan's web trading suite (Dhan Web, the TradingView terminal and Options
 * Trader) is a React app whose order placement is a sticky "Order Window"
 * reached from a watchlist row, an option chain or a chart's Buy/Sell (B/S)
 * buttons. Like Upstox Pro and Groww it ships UTILITY / HASHED class names that
 * change between deploys, so a literal class selector is worthless across
 * builds. Everything here therefore anchors on what survives a rebuild:
 ...
 *     (`[class*="orderWindow"]`, `[class*="tradingSymbol"]`) only as a last
 *     resort.
 *
 * Real Dhan sits behind a login, so these selectors are best-effort by design
 ...
 * a journal — exactly like the Kite, Upstox and Groww adapters.
 *
 * Dhan is derivatives-heavy (it ships a dedicated Options Trader), so most
 * captures are F&O. Dhan renders contract names two ways the app's
 * `parseContractName` already understands: a COMPACT tradingsymbol
 * ("NIFTY24AUG24000CE", "BANKNIFTY24JUN52000CE") and, on the TradingView
 * terminal, a SPACED security name ("NIFTY 25 JUN 2026 24500 CALL"). The
 * normalizer preserves both shapes and only strips decorations + standalone
 * exchange tokens.
 *
 * Bump DHAN_ADAPTER_VERSION whenever these selectors change — it travels with
 * every capture as `adapterVersion`.
 */
export const DHAN_ADAPTER_VERSION = 1;

/* ── Pure text parsing (unit-tested in dhan.test.ts) ─────────────────────── */

/**
 * Dhan order-window instrument text → something the app's contract-name parser
 * (`parseContractName`) understands. Dhan renders names a few ways — a compact
 * tradingsymbol ("NIFTY24AUG24000CE", "RELIANCE"), an exchange-prefixed name
 * ("NSE: RELIANCE", "NSE_EQ|RELIANCE") or (the TradingView terminal case) a
 * SPACED derivative name ("NIFTY 25 JUN 2026 24500 CALL") — so we: uppercase,
 * take the part after any pipe key, drop a leading "<EXCHANGE>:" /
 ...
 */
export function normalizeDhanInstrumentText(raw: string): string {
  let s = raw.toUpperCase().trim();
  // Dhan instrument keys occasionally arrive pipe-keyed ("NSE_EQ|RELIANCE",
  // "NSE_FNO|NIFTY..."). Take the part after the pipe — the human/tradingsymbol.
  if (s.includes("|")) {
  ...
  const cleaned = s
    ...
    .trim();
  if (!cleaned) return "";
  // A compact tradingsymbol with no spaces (RELIANCE, NIFTY24AUG24000CE) is
...
/**
 * Exchange text → a known exchange token, or null. Dhan renders the segment a
 * few ways ("NSE", "NSE_EQ", "NSE • Equity", "NSE F&O", "BSE FNO"), so scan for
 * the first recognized exchange code rather than assuming the whole string is one.
 */
export function normalizeDhanExchange(raw: string): string | null {
...
/**
 * Buy/sell from the order window. Dhan has no single stable class, so layer:
 * a buy/sell class fragment on the window → the active side tab's text → the
 * order button copy ("BUY" / "SELL" / "Buy RELIANCE" / "Place buy order").
 * Ambiguous or missing → null (never guess a trade direction).
 */
export function resolveDhanSide(
...
export interface RawDhanPanelFields {
  symbolText: string;
  exchangeText: string;
  qtyText: string;
  priceText: string;
...
export function assembleDhanCapture(fields: RawDhanPanelFields): CapturedOrder | null {
  const symbol = normalizeDhanInstrumentText(fields.symbolText);
  if (!symbol) return null;
  const side = resolveDhanSide(fields.panelClasses, fields.activeTabText, fields.submitText);
  if (!side) return null;
  ...
  return {
    ...
    symbol,
    exchange: normalizeDhanExchange(fields.exchangeText),
    side,
...
/** name/id/testid attr first, then label-text anchoring (Dhan classes are hashed). */
...
function collectRawFields(panel: HTMLElement): RawDhanPanelFields {
  ...
  return {
    symbolText: firstText(panel, [
      // Dhan idioms (hashed/utility classes, public research): the order window
...
export const dhanAdapter: BrokerCaptureAdapter = {
  id: "dhan",
  label: "Dhan",
  version: DHAN_ADAPTER_VERSION,
  // Dhan Web (web.dhan.co), the TradingView terminal (tv.dhan.co) and Options
  // Trader (options.dhan.co) all live under *.dhan.co; the user grants the host once.
  originPattern: "https://*.dhan.co/*",
  contentScript: "content-dhan.js",
  ...
  readOrder(panel: HTMLElement): CapturedOrder | null {
    try {
      return assembleDhanCapture(collectRawFields(panel));
    } catch {
