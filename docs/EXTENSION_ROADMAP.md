...
- [x] **Upstox adapter** (`extension/src/brokers/upstox.ts`) on the registry. _(Shipped — see "Shipped by the loop".)_
- [x] **Groww adapter** (`extension/src/brokers/groww.ts`) — DOM-capture path (NOT the API path, which would need an owner-procured key). _(Shipped — see "Shipped by the loop".)_
- [x] **Dhan adapter** (`extension/src/brokers/dhan.ts`) — completes the 4-broker set. _(Shipped — see "Shipped by the loop".)_
- [x] **Fyers adapter** (`extension/src/brokers/fyers.ts`) — NSE:SBIN-EQ symbols map to the existing quick-log parser. _(Shipped — see "Shipped by the loop".)_
- [x] **Rules nudge badge** — `chrome.action.setBadgeText` shows unticked-rule count when the day has trades; chrome.alarms-driven refresh. _(Shipped — see "Shipped by the loop".)_
...
- 2026-06-13 — **Upstox order-window capture adapter** — second adapter on the v2 registry. `extension/src/brokers/upstox.ts` (`upstoxAdapter`, id `upstox`, originPattern `https://*.upstox.com/*`, content bundle `content-upstox.js`): one-click "Log in TradeMarkk" prefill from the Upstox Pro Floating Order Window, exactly like Kite. Upstox Pro is a React app with **CSS-Modules hashed class names** (public DOM research; real Upstox is login-walled), so the adapter anchors on what survives a rebuild — visible "Qty"/"Price" label-text walked to the adjacent input, buy/sell class FRAGMENT + active side-tab text + "Confirm to buy/sell" button copy for the side, and `[class*='_symbol_']`/`[class*='ltp']` substring matchers as last resort. Pure `assembleUpstoxCapture()` (+ `normalizeUpstoxInstrumentText` handling Upstox's `NSE_EQ|SYMBOL` pipe-keys & exchange prefixes, `normalizeUpstoxExchange`, `resolveUpstoxSide`) split from DOM collection; market orders fall back to the visible LTP; unknown side/symbol ⇒ no capture (never guess). Registered in `brokers/index.ts` so the Settings toggle, optional host-permission request, dynamic registration, panel prefill and `releaseHostIfUnused` pick it up automatically. New `extension/src/content/upstox-capture.ts` + `vite.content-upstox.config.ts` (4th `ext:build` pass). Committed `upstox-order-window.html` + changed-DOM fixtures; +27 unit tests (490 total); 6 new e2e steps (registration, buy-limit prefill→journal, sell-market last-price fallback, changed-DOM silent degradation). Manual logging + Kite capture + Kite positions-import unchanged. (PR #50)
- 2026-06-13 — **Groww order-window capture adapter** — third adapter on the v2 registry. `extension/src/brokers/groww.ts` (`growwAdapter`, id `groww`, originPattern `https://groww.in/*`, content bundle `content-groww.js`): one-click "Log in TradeMarkk" prefill from the Groww web order pad, exactly like Kite/Upstox. Groww's web terminal ("915") is a React app with **hashed/utility class names** (public DOM research; real groww.in is login-walled), so the adapter anchors on what survives a rebuild — visible "Qty"/"Price" label-text walked to the adjacent input, a buy/sell class FRAGMENT + active side-tab text + order button copy ("Buy …" / "Place sell order") for the side, and `[class*='contractName']`/`[class*='ltp']` substring matchers as last resort. Pure `assembleGrowwCapture()` (+ `normalizeGrowwInstrumentText` — Groww renders F&O as SPACED contract names like "NIFTY 25 JUN 2026 24500 CALL" that `parseContractName` understands directly, plus pipe-keyed/exchange-prefixed equity names; `normalizeGrowwExchange`, `resolveGrowwSide`) split from DOM collection; market orders fall back to the visible LTP; unknown side/symbol ⇒ no capture (never guess). Registered in `brokers/index.ts` so the Settings toggle, optional host-permission request (Groww is covered by the existing `optional_host_permissions:["https://*/*"]` — no manifest change), dynamic registration, panel prefill and `releaseHostIfUnused` pick it up automatically. New `extension/src/content/groww-capture.ts` + `vite.content-groww.config.ts` (5th `ext:build` pass). Committed `groww-order-window.html` + changed-DOM fixtures; +29 unit tests (557 total); 6 new e2e steps (registration, buy-limit prefill→journal, sell-market last-price fallback with spaced-contract parsing, changed-DOM silent degradation). Manual logging + Kite/Upstox capture + Kite positions-import unchanged. (PR #53)
- 2026-06-13 — **Dhan order-window capture adapter** — fourth adapter on the v2 registry, completing the four-broker set. `extension/src/brokers/dhan.ts` (`dhanAdapter`, id `dhan`, originPattern `https://*.dhan.co/*` covering Dhan Web `web.dhan.co`, the TradingView terminal `tv.dhan.co` and Options Trader `options.dhan.co`, content bundle `content-dhan.js`): one-click "Log in TradeMarkk" prefill from Dhan's sticky Order Window, exactly like Kite/Upstox/Groww. Dhan's web suite is a React app with **hashed/utility class names** (public DOM research; real Dhan is login-walled), so the adapter anchors on what survives a rebuild — visible "Qty"/"Price" label-text walked to the adjacent input, a buy/sell class FRAGMENT + active side-tab text + order button copy ("Buy …" / "Place sell order") for the side, and `[class*='tradingSymbol']`/`[class*='ltp']` substring matchers as last resort. Pure `assembleDhanCapture()` (+ `normalizeDhanInstrumentText` — Dhan is derivatives-heavy and renders F&O as COMPACT tradingsymbols like "NIFTY24AUG24000CE" that `parseContractName` understands directly, plus the TradingView terminal's SPACED contract names and pipe-keyed/exchange-prefixed equity; `normalizeDhanExchange`, `resolveDhanSide`) split from DOM collection; market orders fall back to the visible LTP; unknown side/symbol ⇒ no capture (never guess). Registered in `brokers/index.ts` so the Settings toggle, optional host-permission request (Dhan is covered by the existing `optional_host_permissions:["https://*/*"]` — no manifest change), dynamic registration, panel prefill and `releaseHostIfUnused` pick it up automatically. New `extension/src/content/dhan-capture.ts` + `vite.content-dhan.config.ts` (6th `ext:build` pass). Committed `dhan-order-window.html` + changed-DOM fixtures; +30 unit tests (587 total); 6 new e2e steps (registration, buy-limit prefill→journal, sell-market last-price fallback with compact-contract parsing, changed-DOM silent degradation). Manual logging + Kite/Upstox/Groww capture + Kite positions-import unchanged. (PR #56)
- 2026-06-13 — **Fyers order-window capture adapter** — fifth adapter on the v2 registry, completing the Kite/Upstox/Groww/Dhan/Fyers set. `extension/src/brokers/fyers.ts` (`fyersAdapter`, id `fyers`, originPattern `https://*.fyers.in/*` covering Fyers Web `login.fyers.in` / `app.fyers.in` / `fyers.in/web` and the Scalper terminal, content bundle `content-fyers.js`): one-click "Log in TradeMarkk" prefill from Fyers' New/Sticky Order Window, exactly like Kite/Upstox/Groww/Dhan. Fyers Web is a React terminal with **hashed/utility class names** (public DOM research; real Fyers is login-walled), so the adapter anchors on what survives a rebuild — visible "Qty"/"Price" label-text walked to the adjacent input, a buy/sell class FRAGMENT + active side-tab text + order button copy ("Buy …" / "Place sell order") for the side, and `[class*='symbolName']`/`[class*='ltp']` substring matchers as last resort. Pure `assembleFyersCapture()` (+ `normalizeFyersInstrumentText` — Fyers uses EXCHANGE-PREFIXED, series-suffixed tickers like "NSE:SBIN-EQ" / "NSE:NIFTY24JUN24500CE" that `parseContractName` already strips the prefix/suffix from, so the normalizer PRESERVES a native Fyers key verbatim and only falls back to exchange-token stripping for bare names; `normalizeFyersExchange` reads the exchange from a separate tag OR the symbol's prefix, `resolveFyersSide`) split from DOM collection; market orders fall back to the visible LTP; unknown side/symbol ⇒ no capture (never guess). Registered in `brokers/index.ts` so the Settings toggle, optional host-permission request (Fyers is covered by the existing `optional_host_permissions:["https://*/*"]` — no manifest change), dynamic registration, panel prefill and `releaseHostIfUnused` pick it up automatically. New `extension/src/content/fyers-capture.ts` + `vite.content-fyers.config.ts` (7th `ext:build` pass). Committed `fyers-order-window.html` + changed-DOM fixtures; +32 unit tests (691 total with parallel-lane growth); 6 new e2e steps (registration, buy-limit prefill→journal with native equity-key parsing, sell-market last-price fallback with native option-key parsing, changed-DOM silent degradation). Manual logging + Kite/Upstox/Groww/Dhan capture + Kite positions-import unchanged. (PR #58)
- 2026-06-13 — **Rules nudge toolbar badge** — a quiet `chrome.action` badge nudging daily rule discipline: it shows the count of UNTICKED daily rules (e.g. "2") with a loss-red tint, but ONLY once the IST day has ≥1 logged trade AND ≥1 unticked rule; it clears when every rule is addressed (any tri-state counts), there are no trades yet, the user is signed out, or the mode is local/unsupported. New `extension/src/lib/badge-sync.ts` holds the PURE decision (`decideBadge`) + an IST-day helper (`istDayKey`, fixed UTC+05:30 so the boundary is host-timezone-independent) + the canonical journal query (`computeBadgeData`) + a strict snapshot shape-check, so all the logic is unit-tested. The side panel — which already holds a journal connection — owns the data: `useBadgeSync` derives the unticked count from the SAME cached `useRules`/`useRuleChecks` queries the RulesCard renders (so a tri-state flip updates the badge INSTANTLY, optimistically, with no extra DB read and no read-after-write race) plus one cheap trades-today `COUNT(*)`, then writes a snapshot to `chrome.storage.session` and pokes the service worker. The dependency-free `sw.js` never touches the DB: it re-applies that cached snapshot on (a) the panel's `tm:badge` message after a trade-save / rule check-off / tradebook import, (b) a low-frequency `chrome.alarms` tick (~60s — never `setInterval`, which dies with the MV3 worker) that also handles the IST day rollover (a snapshot from a previous day clears), and (c) SW install/startup. Sign-out clears the badge AND cancels the alarm (re-armed on the next sign-in). Manifest gains the `alarms` permission (no host changes). +15 unit tests (706 total with parallel-lane growth); 3 new extension e2e steps assert the live SW badge text via `chrome.action.getBadgeText` (shows "6" once a trade + 6 untouched rules exist, decrements to "5" when one rule is ticked, clears on sign-out). Manual logging + all five capture adapters + Kite positions-import unchanged. (PR #PLACEHOLDER)
